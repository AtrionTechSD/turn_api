...
`id` INT PRIMARY KEY AUTO_INCREMENT NOT NULL,
`name` VARCHAR (75) NOT NULL,
`grade` ENUM ('Grado','Maestría','Doctorado','Diplomado','Otro') NOT NULL DEFAULT 'Grado',
`createdAt` TIMESTAMP NOT NULL DEFAULT current_timestamp,
`updatedAt` TIMESTAMP NOT NULL DEFAULT current_timestamp,
`deletedAt` TIMESTAMP);
...
ALTER TABLE `orders` ADD CONSTRAINT `oorders_client_id_users_id` FOREIGN KEY (`client_id`) REFERENCES `users`(`id`) ON DELETE CASCADE;
